...
function errCalculate(num_spec, PRED_FOLDER, TARGET_FOLDER)
...
for specID = 1 : num_spec
    ...
    SPEC_FOLDER = ['simspec_' num2str(specID) '_' PRED_FOLDER];
...
% 
% % ############# Calculate each rmse of initial values #############
% %% Param define
% PRED_FOLDER = 'sds_5_8_11';
...
% 
% %  error file name
% csvfid_mae = 'mu_mae.csv';
% csvfid_rmse = 'mu_rmse.csv';
% header={'mua1', 'mus1', 'mua2', 'mus2', 'mua3', 'mus3', 'mua4', 'mus4'};
% 
% for specID = 1 : num_spec
%     error_mae = zeros(num_x0, num_mu);
%     error_rmse = zeros(num_x0, num_mu);
%     temp_target = load(fullfile(TARGET_FOLDER, ['param_' num2str(specID+offset) '.txt']));
%     mu_target = temp_target(:, 2:end);
%     SPEC_FOLDER = ['simspec_' num2str(specID+offset) '_' PRED_FOLDER];
%     for i = 1 : num_x0
%         FIT_FOLDER = ['fit_all' num2str(i)];
%         temp_pred = load(fullfile(SPEC_FOLDER, FIT_FOLDER, 'lsqMu_final.txt'));
%         mu_predict = temp_pred(:, 2:end);
% 
%         % calculate error
%         error = ((mu_predict - mu_target) ./ mu_target)*100;
%         error_mae(i, :) = mean(abs(error), 1);
%         error_rmse(i, :) = sqrt(mean(error.^2, 1));
%     end
%     table_mae = array2table(error_mae, 'VariableNames', header);
%     table_rmse = array2table(error_rmse, 'VariableNames', header);
%     writetable(table_mae, fullfile(SPEC_FOLDER, csvfid_mae));
%     writetable(table_rmse, fullfile(SPEC_FOLDER, csvfid_rmse));
% end
% % ###############################################################
...
% %%  Calculate Error
% error = ((mu_predict - mu_target) ./ mu_target)*100;
% error_per_mean = squeeze(mean(abs(error), 2));
% error_all_mean = squeeze(mean(mean(abs(error), 2), 1));
% % error_per_std = std(squeeze(error))';
% error_rms = sqrt(mean(((mu_predict - mu_target) ./ mu_target).^2, 2));
% error_per_rmsp = squeeze(error_rms*100);
% error_all_rmsp_mean = squeeze(mean(error_rms, 1)*100);
% % error_all_rmsp_std = squeeze(std(error_rms, 1)*100);
% 
% tissue_name = {'mua1', 'mus1', 'mua2','mus2','mua3','mus3','mua4','mus4'};
% 
% % write each spec error
% for i = 1:size(error_per_mean, 1)
%     fid = fopen(fullfile(PRED_FOLDER, ['error_' num2str(i+offset) '.txt']), "w");
%     fprintf(fid, 'Tissue \tMean \t\t Rmsp \t\t Std\n');
%     for j = 1:size(error_per_mean, 2)
%         fprintf(fid, [tissue_name{j} '\t']);
%         fprintf(fid, '%8.4f\t', error_per_mean(i, j));
%         fprintf(fid, '%8.4f\t', error_per_rmsp(i, j));
% %         fprintf(fid, '%8.4f\t', error_per_std(j));
%         fprintf(fid, '\n');
%     end
%     fclose(fid);
% end
% 
% % write all error
% fid = fopen(fullfile(PRED_FOLDER, 'error_all.txt'), "w");
% fprintf(fid, 'Tissue \t Mean \t\t Rmsp \t\t Std(rmsp)\n');
% for j = 1:size(error_all_mean, 1)
%     fprintf(fid, [tissue_name{j} '\t']);
%     fprintf(fid, '%8.4f\t', error_all_mean(j));
%     fprintf(fid, '%8.4f\t', error_all_rmsp_mean(j));
% %     fprintf(fid, '%8.4f\t', error_all_rmsp_std(j));
%     fprintf(fid, '\n');
% end
% fclose(fid);
% 
% % fid = fopen('mu/mean.txt','w');
% % for i = 1:size(error_all_mean, 1)
% %     fprintf(fid,'%.4f ', error_all_mean(i));
% %     fprintf(fid,'\n');
% % end
% % fclose(fid);
% % 
% % fid = fopen(fullfile(PRED_FOLDER, 'std.txt'),'w');
% % for i = 1:size(error_per_std, 1)
% %     fprintf(fid,'%.4f ', error_per_std(i));
% %     fprintf(fid,'\n');
% % end
% % fclose(fid);
% % 
% % fid = fopen('mu/rmsp.txt','w');
% % for i = 1:size(error_all_rmsp_mean, 1)
% %     fprintf(fid,'%.4f ', error_all_rmsp_mean(i));
% %     fprintf(fid,'\n');
% % end
% % fclose(fid);
% % 
% % fid = fopen('mu/rmsp_std.txt','w');
% % for i = 1:size(error_all_rmsp_std, 1)
% %     fprintf(fid,'%.4f ', error_all_rmsp_std(i));
% %     fprintf(fid,'\n');
% % end
% % fclose(fid);
% 
...
% 
% for i = 1:num_spec
%     scatter(wl, error(i, :, 1));
%     hold on
% end
% title('μa_{epidermis}  error');
% legend('Location','northeastoutside');
% xlabel('Wavelength');
% ylabel('error (%)');
% saveas(gcf,'mu/μa_epi.png');
% close(gcf);
% 
% %%
% for i = 1:num_spec
%     scatter(wl, error(i, :, 2));
%     hold on
% end
% title('μs_{epidermis}  error');
% legend('Location','northeastoutside');
% xlabel('Wavelength');
% ylabel('error (%)');
% saveas(gcf,'mu/μs_epi.png');
% close(gcf);
% 
% %%
% for i = 1:num_spec
%     scatter(wl, error(i, :, 3));
%     hold on
% end
% title('μa_{dermis}  error');
% legend('Location','northeastoutside');
% xlabel('Wavelength');
% ylabel('error (%)');
% saveas(gcf,'mu/μa_dermis.png');
% close(gcf);
% 
% %%
% for i = 1:num_spec
%     scatter(wl, error(i, :, 4));
%     hold on
% end
% title('μs_{dermis}  error');
% legend('Location','northeastoutside');
% xlabel('Wavelength');
% ylabel('error (%)');
% saveas(gcf,'mu/μs_dermis.png');
% close(gcf);
% 
% %%
% for i = 1:num_spec
%     scatter(wl, error(i, :, 5));
%     hold on
% end
% title('μa_{subcutaneous}  error');
% legend('Location','northeastoutside');
% xlabel('Wavelength');
% ylabel('error (%)');
% saveas(gcf,'mu/μa_subcaneous.png');
% close(gcf);
% 
% %%
% for i = 1:num_spec
%     scatter(wl, error(i, :, 6));
%     hold on
% end
% title('μs_{subcutaneous}  error');
% legend('Location','northeastoutside');
% xlabel('Wavelength');
% ylabel('error (%)');
% saveas(gcf,'mu/μs_subcaneous.png');
% close(gcf);
% 
% %%
% for i = 1:num_spec
%     scatter(wl, error(i, :, 7));
%     hold on
% end
% title('μa_{muscle}  error');
% legend('Location','northeastoutside');
% xlabel('Wavelength');
% ylabel('error (%)');
% saveas(gcf,'mu/μa_muscle.png');
% close(gcf);
% 
% %%
% for i = 1:num_spec
%     scatter(wl, error(i, :, 8));
%     hold on
% end
% title('μs_{muscle}  error');
% legend('Location','northeastoutside');
% xlabel('Wavelength');
% ylabel('error (%)');
% saveas(gcf,'mu/μs_muscle.png');
% close(gcf);
